...
def read_holidays(holiday_file_path: str) -> set:
    ...
    try:
        ...
        try:
            with open(holiday_file_path, 'r', encoding='shift_jis',
                      newline='') as f:
                reader = csv.reader(f)
                for row in reader:
                    if row:
...
def calculate_schedule(config: dict, base_path: str) -> list:
    ...
    try:
        start_date = datetime.strptime(config['schedule_period']['start_date'],
                                       '%Y-%m-%d').date()
        end_date = datetime.strptime(config['schedule_period']['end_date'],
                                     '%Y-%m-%d').date()
    ...
    while current_date <= end_date:
        ...
        if is_weekday and is_not_holiday:
            for daily_task in config.get('daily_schedules', []):
                try:
                    task_time = datetime.strptime(daily_task['time'],
                                                  '%H:%M:%S').time()
                    ...
                    print(f"警告: daily_schedules内のタスク定義が不正なためスキップします。"
                          f": {daily_task} - {e}")
    ...
    print(f"情報: {start_date} から {end_date} までの期間で、"
...
if __name__ == '__main__':
    ...
    dummy_config = {
        "schedule_period": {"start_date": "2025-06-09",
                            "end_date": "2025-06-13"},
        "holiday_list_path": "holidays_test.csv",
        "daily_schedules": [{"time": "09:00:00", "task_type": "test",
                             "task_path": "test.exe"}]
    }
    with open(config_path, 'w', encoding='utf-8') as f:
        json.dump(dummy_config, f, indent=2)
    with open(os.path.join(base_path, 'holidays_test.csv'), 'w',
    ...
    if config_data:
        ...
        for task in full_schedule[:5]:
            print(f"{task['datetime'].strftime('%Y-%m-%d %H:%M:%S (%a)')} - "
